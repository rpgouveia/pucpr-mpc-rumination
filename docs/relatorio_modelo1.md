...
Replicar o experimento de Pavlovic et al. (2021) utilizando uma Rede Neural Convolucional unidimensional (CNN 1D) como **baseline** para comparação posterior com o Modelo 2 (Random Forest).
...
Undersampling por animal para igualar a representação das 3 classes no conjunto de treino.
...
A maior fonte de erro é a **confusão entre Ruminação e Comendo (22%)**, fenômeno documentado pelo artigo original: *"eating and rumination are characterised by similar jaw motions"*. Esse padrão será observado comparativamente no Modelo 2.

### 7.4 Variabilidade entre folds

O Fold 5, com 4 animais na validação, apresentou maior ruído nas curvas de aprendizado e o menor F1 (0,7776). Isso evidencia que a **variabilidade inter-animal** é o principal desafio do problema — cada animal tem padrões comportamentais ligeiramente distintos.
...
Treinamento e avaliação do **Modelo 2 — Random Forest** (`notebooks/model2.ipynb`), com features estatísticas extraídas das mesmas janelas de 90 s, sobre o mesmo conjunto de treino e teste, para comparação direta com os resultados acima.
